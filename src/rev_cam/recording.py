...
class _ActiveChunkWriter:
    ...
    def add_frame(self, array: "_np.ndarray", timestamp: float) -> None:
        ...
        if self.first_timestamp is None:
            base_timestamp = timestamp_value if timestamp_value is not None else 0.0
            self.first_timestamp = base_timestamp
        else:
            base_timestamp = float(self.first_timestamp)

        if self.fps > 0:
            expected_offset = float(self.frame_count) / float(self.fps)
        else:
            expected_offset = float(self.frame_count)
        expected_timestamp = base_timestamp + expected_offset

        if timestamp_value is None:
            effective_timestamp = expected_timestamp
        else:
            effective_timestamp = max(timestamp_value, expected_timestamp)

        self.last_timestamp = effective_timestamp
