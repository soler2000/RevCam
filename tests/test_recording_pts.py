...
def test_chunk_writer_pts_are_relative(tmp_path):
    ...
    assert stream.encoded_pts == [0, 1, 2]
...
def test_chunk_writer_enforces_configured_fps(tmp_path):
    path = tmp_path / "chunk.mp4"
    ...
    for index in range(10):
        writer.add_frame(frame, timestamp=index * 0.01)

    entry = writer.finalise()

    assert entry["frame_count"] == 10
    assert stream.encoded_pts == list(range(10))
    assert entry["duration_seconds"] == pytest.approx(10 / float(frame_rate), abs=1e-3)
